...
var process = function(model, sentences, languages, alphaValue, randomSeed) {
  ...
  for(var i=0;i<vocab.length;i++) {
    f[vocab[i]] = 1;
    console.log('vocab['+i+'] = '+vocab[i]);
    vocabOrig[vocab[i]] = vocab[i];
  }
  // Array of stop words
  ...
  if (sentences && sentences.length > 0) {
    ...
    lda_perplexity.configure(documents,V, 10, 2000, 100, 10, randomSeed);
...
var lda_perplexity = new function() {
    var documents,z,nw,nwsum,V,K,alpha,beta; 
    ...
    this.left_to_right_sampling = function(m) {
      ...
      for(var limit=0;limit<docLength;limit++) {
        ...
        console.log('sample on position: '+limit+', word='+this.documents[m][limit]+', prob='+prob);
